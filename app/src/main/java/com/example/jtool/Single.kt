...
class JapaneseSingle : Single() {
    ...
    fun searchSongs(titleKeyword:String): ArrayList<Array<Any>> {
        ...
        for ( i in resultTable.indices.drop(1) ) {
            val songTitle  = resultTable[i].select("td[class=side td1]").text()
            val artist     = resultTable[i].select("td[class=td2]").text()
            val lyricsInfo = resultTable[i].select("td[class=side td5]").text()
            val songNumber = resultTable[i].select("a[href]").attr("href")
